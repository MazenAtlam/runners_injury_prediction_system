...
# READ ALL (With optional session_id filter)
@sensor_data_bp.route('/', methods=['GET'])
def get_all_sensor_data():
    session_id = request.args.get('session_id')

    if session_id:
        data_list = SensorData.query.filter_by(session_id=session_id).all()
    else:
...
def get_sensor_data_entry(id):
    d = SensorData.query.get_or_404(id)
    return jsonify({
        'id': d.id,
        'session_id': d.session_id,
...
def update_sensor_data(id):
    ...
    try:
        ...
        d.updated_by = data.get('updated_by', 'API_USER')
